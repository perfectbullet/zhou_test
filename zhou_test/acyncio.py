...
def segment_TGSE(file_path):
    ...
    for cnt in contours:
        ...
        # np.savetxt(file_path.replace('.jpg', '-' + str(cnt.shape[0]) + '.txt'), np.reshape(cnt, (cnt.shape[0], 2)), delimiter=',', fmt='%d')
    ...
    for cnt in contours2:
        ...
        if cy < h * 0.2 or cy > h * 0.8:
            continue
        # np.savetxt(file_path.replace('.jpg', '-' + str(cnt.shape[0]) + '.txt'), np.reshape(cnt, (cnt.shape[0], 2)),
        #            delimiter=',', fmt='%d')
    ...
    # yield close_plt(plt, file_path)
    yield plt.savefig(file_path.replace('.jpg', '.png'), dpi=300)  # 慢
    # plt.close('all')  # 关闭图 0
    t3 = time.time()
